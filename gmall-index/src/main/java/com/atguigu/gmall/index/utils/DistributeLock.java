...
public class DistributeLock {
    ...
    private void renewTime(String lockName,String uuid,Long expire){
        //判断是否有自己的锁
        String script = "if(redis.call('hexists',KEYS[1],ARGV[1]) == 0) then return redis.call('expire',KEYS[1],ARGV[2]) end ;";
        this.thread = new Thread(() ->{
            while (true){
                try {
                    Thread.sleep(expire * 1000 * 2 / 3);
                    this.redisTemplate.execute(new DefaultRedisScript<>(script,Long.class),Arrays.asList(lockName),uuid);
